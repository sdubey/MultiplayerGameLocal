 ...
 * @author Gary
 */
public final class BirthListDAO {

    private BirthListDAO() {
    }

    public static void createEntry(int zone_id, int species_id, int total) throws SQLException {
        String query_1 = "SELECT * FROM `birth_list` WHERE `zone_id` = ? AND `species_id` = ?";
        ...
        try {
            ...
            if (!rs.next()) {
                ...
                String query_2 = "INSERT INTO `birth_list` (`zone_id`, `species_id`, `total`) VALUES (?, ?, ?)";
                ...
                String query_3 = "UPDATE `birth_list` SET `total` = ? WHERE `zone_id` = ? AND `species_id` = ?";
    ...
    public static void removeEntry(int zone_id, int species_id) throws SQLException {
        String query = "DELETE FROM `birth_list` WHERE `zone_id` = ? AND `species_id` = ?";
    ...
    public static HashMap<Integer, Integer[]> getList(int zone_id) throws SQLException {
        ...
        String query = "SELECT * FROM `birth_list` WHERE `zone_id` = ?";
    ...
    public static void updateAmount(int zone_id, int species_id, int amount) throws SQLException {
        String query = "UPDATE `birth_list` SET `amount` = ? WHERE `zone_id` = ? AND `species_id` = ?";
